...
class GuidedFilterImpl {
 public:
  ...
  cv::cuda::GpuMat filter(const cv::cuda::GpuMat &p, int depth);
...
cv::Mat guidedFilter(const cv::Mat &guide,
                              const cv::Mat &src,
                              int radius,
                              double eps,
                              int dDepth) {
  cv::cuda::GpuMat guide_gpu(guide);
  cv::cuda::GpuMat src_gpu(src);
  cv::Mat result(GuidedFilter(guide_gpu, radius, eps).filter(src_gpu, dDepth));
  return result;
